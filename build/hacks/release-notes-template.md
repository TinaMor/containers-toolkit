...
```PowerShell
Install-Module -Name Containers-Toolkit -RequiredVersion "__TRIMMED_VERSION__" __ALLOW_PRERELEASE__
```

If the module is already installed, update the module:

```PowerShell
Update-Module -Name Containers-Toolkit -RequiredVersion "__TRIMMED_VERSION__" __ALLOW_PRERELEASE__
```

### Download Source Files

> **⚠️ Caution:**  
> For security, we recommend using the source files named `containers-toolkit-__REQUIRED_VERSION__.tar.gz`, as these contain signed scripts.
...
For any questions or feedback on this release, see the discussion: [Containers.ToolKit **TRIMMED_VERSION**](<LINK-TO-VERSION-DISCUSSION>)
